...
public class OrdinalScale extends RestrictedScale {
	...
	protected boolean restrictionIsValid(String leafName,
		...
		if (getRestriction() == null || "".equals(getRestriction())) {
			errors.add(new ValidationError(
					"Please enter a restriction for the scale of type 'Ordinal' at leaf '"
							+ leafName + "'", this));
			return false;
		}
		...
		if (values.size() < 2) {
			errors.add(new ValidationError(
					"At least 2 possible ordinal values have to be defined for leaf \""
							+ leafName + "\"", this));
			return false;
		}
		for (int i = 0; i < values.size(); i++) {
			if (values.get(i).length() == 0) {
				errors.add(new ValidationError(
						"The empty String is not a valid value for an Ordinal Scale (used in leaf \""
								+ leafName + "\")", this));
				return false;
	...
	public String getReadableRestriction() {
		...
		return niceFormat.substring(0, lastIndex) + " or "
				+ niceFormat.substring(lastIndex + 2);
	}
	...
	public boolean isEvaluated(Value value) {
		...
		if ((value != null)
				&& (value.getScale().getType() == ScaleType.ordinal)) {
