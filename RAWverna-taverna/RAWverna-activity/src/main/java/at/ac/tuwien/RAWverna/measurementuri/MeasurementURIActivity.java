...
public abstract class MeasurementURIActivity<C> extends
		AbstractAsynchronousActivity<C> {
	 ...
	 */
	protected List<MeasurementInfoUri> measurementInfoUris = new ArrayList<MeasurementInfoUri>(
			20);
	 ...
	 */
	protected Map<String, T2Reference> registerOutputs(
		...
		for (Entry<MeasurementInfoUri, Value> entry : results.entrySet()) {
			...
			if (null == entry.getValue()) {
				...
				ErrorDocument errorDoc = referenceService
						.getErrorDocumentService().registerError(
								"Measurement returned null: "
										+ entry.getValue().getComment(), 0,
								context);
				reference = referenceService.register(errorDoc, 0, true,
						context);
			} else {
				// Register value
				reference = referenceService.register(entry.getValue()
