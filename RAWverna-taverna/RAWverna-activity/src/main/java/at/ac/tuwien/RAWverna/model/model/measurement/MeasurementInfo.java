...
import at.ac.tuwien.RAWverna.model.model.tree.CriterionCategory;
...
public class MeasurementInfo implements Serializable, Cloneable {
	...
	public void fromUri(String uri) {
		...
		if ((pathIdx == -1) || ((pathIdx + 1) >= path.length())) {
			throw new IllegalArgumentException(
					"invalid measurment info uri - no path defined: " + uri);
		}
		...
		if (cat == null) {
			throw new IllegalArgumentException(
					"invalid measurement info uri - scheme and path don't correspond to a criterion category: "
							+ uri);
		}
	...
	public String getUri() {
		...
		return scheme + "://" + path + "/" + property.getPropertyId()
				+ fragment;
	}
