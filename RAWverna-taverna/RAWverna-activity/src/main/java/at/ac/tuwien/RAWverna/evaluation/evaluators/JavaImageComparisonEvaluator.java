 ...
 * @version 1.0
 */
public class JavaImageComparisonEvaluator extends EvaluatorBase implements
		IObjectEvaluator {
	...
	private static Logger logger = Logger
			.getLogger(JavaImageEvaluatorActivity.class);
	...
	public HashMap<MeasurementInfoUri, Value> evaluate(BufferedImage image1,
			BufferedImage image2, List<MeasurementInfoUri> measurementInfoUris)
			throws EvaluatorException {

		try {
			HashMap<MeasurementInfoUri, Value> results = new HashMap<MeasurementInfoUri, Value>();

			ConvenientBufferedImageWrapper sampleImg = new ConvenientBufferedImageWrapper(
					image1);
			SRGBColorConverter sampleImgColor = null;
			ConvenientBufferedImageWrapper resultImg = new ConvenientBufferedImageWrapper(
					image2);
			...
			for (MeasurementInfoUri measurementInfoUri : measurementInfoUris) {
				...
				if ((propertyURI != null)
						&& propertyURI
								.startsWith(OBJECT_IMAGE_SIMILARITY + "#")) {
			...
			for (MeasurementInfoUri measurementInfoUri : orderedMeasurements) {
				...
				Scale scale = descriptor
						.getMeasurementScale(measurementInfoUri);
				...
				if (fragment.equals("equal")) {
					...
					if (null == p1) {
						...
						v.setComment("Images are not the same as image size differs ("
								+ sampleImg.getWidth()
								+ "x"
								+ sampleImg.getHeight()
								+ " vs. "
					...
					try {
						metric = new EqualMetric(
								new AutoColorConverter(
										sampleImg,
										resultImg,
					...
					v.setComment("Java ImageCompare returned images are equal: "
							+ b);
					...
					if (fragment.startsWith("ssimSimple")) {
						...
						try {
							...
							ConvenientBufferedImageWrapper scaledSample = new ConvenientBufferedImageWrapper(
									SimpleSSIMMetric.prepare(sampleImgColor
							...
							ConvenientBufferedImageWrapper scaledResult = new ConvenientBufferedImageWrapper(
									SimpleSSIMMetric.prepare(resultImgColor
											.getImage().getBufferedImage(),
											Math.max(0, -dx), Math.max(0, -dy),
											p1.x, p1.y, 512));

							metric = new SimpleSSIMMetric(
									new HSBColorConverter(
											new SRGBColorConverter(scaledSample)),
									new HSBColorConverter(
											new SRGBColorConverter(scaledResult)),
									p0, new Point(scaledSample.getWidth(),
											scaledSample.getHeight()));
						} catch (ImageException e) {
							logger.error(e);
							continue;
						}
						...
						if (-1 == selectChannel) {
							...
							d = Double.valueOf(op.getResult().getChannelValue(
									selectChannel));
						}
						...
						v.setComment("Java ImageCompare returned "
								+ Double.toString(d) + " for " + fragment);
						...
						if (fragment.equals("aeRelative")) {
							metric = new AEMetric(sampleImgColor,
									resultImgColor, p0, p1);
						} else if (fragment.equals("paeRelative")) {
							metric = new PAEMetric(sampleImgColor,
									resultImgColor, p0, p1);
						} else if (fragment.equals("maeRelative")) {
							metric = new MAEMetric(sampleImgColor,
									resultImgColor, p0, p1);
						} else if (fragment.equals("mseRelative")) {
							metric = new MSEMetric(sampleImgColor,
									resultImgColor, p0, p1);
						}
						/*-
						...
						if (null != metric) {
							TransientOperation<Float, StaticColor> op = metric
									.execute();
							Double d;
							...
							if (-1 == selectChannel) {
								...
								d = Double.valueOf(op.getResult()
										.getChannelValue(selectChannel));
							}
	...
	private Point getSize(ConvenientBufferedImageWrapper img1,
			ConvenientBufferedImageWrapper img2) {
	...
	private Point getTolerantSize(ConvenientBufferedImageWrapper img1,
			ConvenientBufferedImageWrapper img2) {
		return new Point(Math.min(img1.getWidth(), img2.getWidth()), Math.min(
				img1.getHeight(), img2.getHeight()));
	}
}
