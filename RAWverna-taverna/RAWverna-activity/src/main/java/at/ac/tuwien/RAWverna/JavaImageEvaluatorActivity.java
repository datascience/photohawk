...
public class JavaImageEvaluatorActivity extends
		MeasurementURIActivity<JavaImageEvaluatorActivityConfigurationBean>
		implements
		AsynchronousActivity<JavaImageEvaluatorActivityConfigurationBean> {
	...
	@Override
	public void configure(JavaImageEvaluatorActivityConfigurationBean configBean)
	...
	public void executeAsynch(final Map<String, T2Reference> inputs,
		...
		callback.requestRun(new Runnable() {
			...
			public void run() {
				...
				BufferedImage image1 = wrapInputImage(callback,
						inputs.get(IN_IMAGE_1));
				if (null == image1) {
					callback.fail("JavaImageEvalutatorActivity: 2 Could not read image on port "
							+ IN_IMAGE_1);
				...
				BufferedImage image2 = wrapInputImage(callback,
						inputs.get(IN_IMAGE_2));
				if (null == image2) {
					callback.fail("JavaImageEvalutatorActivity: 2 Could not read image on port "
							+ IN_IMAGE_2);
				...
				try {
					...
					results = evaluator.evaluate(image1, image2,
							measurementInfoUris);
				} catch (EvaluatorException e) {
					logger.error("Error evaluating " + IN_IMAGE_1 + " and "
							+ IN_IMAGE_2, e);
					callback.fail(
							"JavaImageEvalutatorActivity: Error evaluating "
									+ IN_IMAGE_1 + " and " + IN_IMAGE_2, e);
					return;
				}
	 ...
	 */
	private BufferedImage wrapInputImage(AsynchronousActivityCallback callback,
		...
		try {
			...
			byte[] imageData = (byte[]) referenceService.renderIdentifier(
					reference, byte[].class, context);
			...
			if (logger.isDebugEnabled()) {
				logger.debug("Image data has size " + imageData.length
						+ " bytes");
			}
			...
			logger.info("Image read with size " + image.getWidth() + "x"
					+ image.getHeight());
		} catch (ReferenceServiceException e) {
			logger.error("Could not get image data from reference service", e);
			callback.fail(
					"JavaImageEvalutatorActivity: Could not get image data from reference service",
					e);
			return null;
		} catch (IOException e) {
			logger.error("Could not wrap input data into BufferdImage", e);
			callback.fail(
					"JavaImageEvalutatorActivity: Could not wrap input data into BufferdImage",
					e);
			return null;
		}
