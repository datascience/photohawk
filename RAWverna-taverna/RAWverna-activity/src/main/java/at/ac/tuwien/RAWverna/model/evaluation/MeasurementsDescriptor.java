...
public class MeasurementsDescriptor {
	private static final Logger log = Logger
			.getLogger(MeasurementsDescriptor.class);
		...
		for (MeasurableProperty p : digestedProperties.values()) {
			...
			if (uri == null) {
				...
				for (Metric m : p.getPossibleMetrics()) {
					...
					if (existingP != null
							&& (!containsMetric(m.getMetricId(),
									existingP.getPossibleMetrics()))) {
	 ...
	 */
	public List<Scale> getMeasurementScales(
			List<MeasurementInfoUri> measurements) {
